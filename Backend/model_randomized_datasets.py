...
#               and RRT use using the derivation COVID dataset. It does the training for the combined + randomly split datasets.
...
def load_dataset(dataset_type):
    return pd.read_csv(f'Backend/Data/randomized_{dataset_type}_derivation_patients.csv')
...
def preprocess_data(df, model, dataset_type):
    preprocessed_data_path = f'Backend/Preprocessed Datasets/preprocessing_{model["name"]}_randomized_{dataset_type}.csv'
    ...
    if os.path.exists(preprocessed_data_path):
        ...
        else: ## If CAP, do not remove non-COVID patients
            df = (df.pipe(process_early_deaths)                         # remove all patients who passed within 72 hours of admission
                    .pipe(remove_unknown_outcome)                       # remove all patients with unknown outcome or who were discharged to another facility
                    .pipe(process_med_columns)                          # remove medication data if patient was NOT on the medication on the day of admission
                    .pipe(remove_non_numeric)                           # remove non-numeric features
...
def train_model(df, model, classifier, dataset_type):
    ...
    model_path = f'{classifier["name"]}_{model["name"]}_randomized_{dataset_type}.pkl'
    ...
    if os.path.exists(model_path):
        ...
        df_params.to_excel(f'Backend/Model Metrics/{classifier["name"]}/{model["name"]}/best_params_{dataset_type}.xlsx', index_label='Parameter')
    
        # Retrieve feature importances
        importance_scores = best_model.feature_importances_
        feature_importances = pd.DataFrame({'Feature': X_train.columns, 'Importance': importance_scores})
        feature_importances = feature_importances.sort_values(by='Importance', ascending=False)
        feature_importances.to_excel(f'Backend/Model Metrics/{classifier["name"]}/{model["name"]}/importances_randomized_{dataset_type}.xlsx', index=False)
    ...
    # Calculate metrics.
    calc_and_write_metrics(y_pred, y_test, probability_positive, model["name"], classifier["name"], f'_randomized_{dataset_type}')
...
# Outputs:      Nil.
def coordinate_pipeline(model, classifier, dataset_type):
    df = load_dataset(dataset_type)
    df_preprocessed = preprocess_data(df, model, dataset_type)
    train_model(df_preprocessed, model, classifier, dataset_type)

# Run the pipeline
coordinate_pipeline(vent, rfc, 'COVID')
